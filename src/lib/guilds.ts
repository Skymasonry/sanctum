...
function rowToGuild(row: GuildRow, members: string[], pending: string[], applications: GuildApplication[], agreements: Array<{ id: number; text: string }>, stewards: string[], leadershipCircle: string[]): Guild {
  return {
    id: row.id,
    name: row.name,
...
async function loadGuilds(where: string, params: unknown[] = []): Promise<Guild[]> {
  ...
  const [members, apps, forms, stewards, leadership] = await Promise.all([
    ...
    db.query<{ guild_id: string; user_id: string }>(
      `SELECT guild_id, user_id FROM guild_leadership_circle WHERE guild_id = ANY($1::text[])`,
      [ids],
    ),
  ])
  ...
  const leadershipByGuild = new Map<string, string[]>()
  for (const l of leadership.rows) {
    const list = leadershipByGuild.get(l.guild_id) ?? []
    list.push(l.user_id)
    leadershipByGuild.set(l.guild_id, list)
  }
...
export async function getUserGuilds(): Promise<Guild[]> {
  ...
  return loadGuilds(
    `WHERE id IN (
       ...
       OR id IN (SELECT guild_id FROM guild_leadership_circle WHERE lower(user_id) = lower($1))`,
    [username],
  )
}
